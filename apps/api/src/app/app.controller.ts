...
*/
  ...
  async redirect(
    ...
    @Query() queryParams: Record<string, string | string[]>,
    @Res() res
  ) {
      ...
      Object.keys(queryParams).forEach((d: string) => {
        if (Array.isArray(queryParams[d])) {
          (queryParams[d] as string[]).forEach((val) => {
            qParamList.push(
              encodeURIComponent(d) + "=" + encodeURIComponent(val)
            );
          });
        } else {
          qParamList.push(
            encodeURIComponent(d) +
              "=" +
              encodeURIComponent(queryParams[d] as string)
          );
        }
      });
  ...
  async register(@Body() link: Link): Promise<LinkModel> {
    ...
    return response.then((createdLink) => {
      this.telemetryService.sendEvent(
        this.configService.get<string>("POSTHOG_DISTINCT_KEY"),
        `Created Link`,
        {
  ...
  async update(
    ...
    return this.appService.updateLink(id, link).then((updatedLink) => {
      this.telemetryService.sendEvent(
        this.configService.get<string>("POSTHOG_DISTINCT_KEY"),
        `Updated Link`,
        {
  ...
  async getNotifications(
    ...
    await this.appService
      .updateClicksInPostgresDB(id)
      .then((res) => {
        console.log("UPDATED IN DB SUCCESS");
      })
      .catch((err) => {
        console.log(err);
      });
