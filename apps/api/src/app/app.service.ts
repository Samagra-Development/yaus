...
export class AppService {
  ...
  async updateClicksInPostgresDB(id: string | number): Promise<void> {
    ...
    Number.isNaN(parseInt(id.toString()))
      ? (id = await this.redisUtils.fetchKey(id.toString()))
      : 0;
  ...
  async createLinkInDB(data: Prisma.linkCreateInput): Promise<link> {
    ...
    if (
      data.customHashId != undefined &&
      !validCustomHashIdRegex.test(data.customHashId)
    ) {
      return Promise.reject(
        new Error(
          "Invalid custom hashId. Only alphanumeric characters, hyphens and underscores are allowed."
        )
      );
    }
  ...
  async resolveRedirect(
    Id: string
  ): Promise<{ reRouteurl: string; redirectedLink: link }> {
    ...
    if (validHashIdRegex.test(Id)) {
      ...
      if (hashId == undefined) {
        ...
        !(linkData == null)
          ? (response = await this.redirect(linkData.hashid.toString()))
          : 0;
        return response;
      } else {
    ...
    return this.redisUtils
      ...
      .catch((err) => {
        this.telemetryService.sendEvent(
          ...
          "Exception in fetching data from redis falling back to DB",
          { error: err.message }
        );
  ...
  async redirectFromDB(
    ...
    return this.prisma.link
      ...
      .then((response) => {
        ...
        if (
          !Number.isNaN(parseInt(params?.["expiry"])) &&
          currentTime > createdAt + 60 * parseInt(params?.["expiry"])
        ) {
